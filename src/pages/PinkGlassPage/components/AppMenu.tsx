...
const AppMenu: React.FC<AppMenuProps> = ({ language }) => {
  ...
  const handleDrop = (e: React.DragEvent, targetGroup: 'favorites' | 'others') => {
    ...
    if (dropTarget) {
      ...
      insertIndex = e.clientY - rect.top > rect.height / 2 ? targetIndex + 1 : targetIndex;
    } else insertIndex = targetList.length;
  ...
  const renderAppItem = (app: AppItem, source: 'favorites' | 'others') => (
    <div key={app.id} data-app-id={app.id} draggable onDragStart={(e) => handleDragStart(e, app.id, source)} className="flex flex-col items-center justify-center p-3 rounded-2xl glass-hover transition-all duration-200 cursor-move group relative active:scale-95 active:opacity-80">
