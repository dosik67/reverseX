...
const BookmarkGrid: React.FC<BookmarkGridProps> = ({ language }) => {
  ...
  return (
    <>
      <div className="mt-12 w-full max-w-5xl px-4 flex justify-center">
        <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 md:gap-8">
            <SortableContext items={bookmarks.map(b => b.id)} strategy={rectSortingStrategy}>
                {bookmarks.map((b) => (
                    <SortableBookmark key={b.id} bookmark={b} variant="card" onRemove={removeBookmark} onEdit={(bookmark) => setEditingBookmark(bookmark)} />
                ))}
            </SortableContext>
            ...
            <form onSubmit={addBookmark} className="space-y-4">
              <div>
                ...
                <input type="text" required value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-[var(--theme-color)] focus:ring-1 focus:ring-[var(--theme-color)] transition-all" placeholder="e.g. YouTube" autoFocus />
              </div>
              <div>
                ...
                <input type="text" required value={newUrl} onChange={(e) => setNewUrl(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-[var(--theme-color)] focus:ring-1 focus:ring-[var(--theme-color)] transition-all" placeholder="e.g. youtube.com" />
              </div>
      ...
      <AnimatePresence>
        {editingBookmark && (
            <EditBookmarkModal key="edit-modal" bookmark={editingBookmark} onClose={() => setEditingBookmark(null)} onSave={updateBookmark} />
        )}
      </AnimatePresence>
    </>
  );
