...
const EditBookmarkModal: React.FC<EditBookmarkModalProps> = ({ bookmark, onClose, onSave }) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        if (file.type === 'image/gif') {
           if (file.size > 1024 * 1024) alert("This GIF is quite large. Syncing might be slower.");
           resolve(result);
           return;
        }
        const img = new Image();
        img.onload = () => {
          ...
          if (ctx) { ctx.drawImage(img, 0, 0, width, height); resolve(canvas.toDataURL(file.type, 0.8)); } else resolve(result);
        };
        img.onerror = reject;
        img.src = result;
      };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          setCustomIconUrl(await processImageFile(file));
      } catch (err) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave(bookmark.id, { title, url, customIconUrl: customIconUrl || undefined, initials: title.substring(0, 2).toUpperCase() });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <motion.div 
        className="w-full max-w-md glass-panel p-6 rounded-3xl shadow-2xl relative border border-white/10 flex flex-col gap-6"
      ...
      >
        <div className="flex justify-between items-center">
            ...
            <button onClick={onClose} className="p-1 rounded-full hover:bg-white/10 text-white/70 hover:text-white transition-colors"><X size={20} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="relative group flex flex-col items-center gap-3">
                    <div className="w-24 h-24 rounded-[2rem] glass-panel flex items-center justify-center border border-white/20 shadow-lg bg-white/5 overflow-hidden relative">
                        {isProcessing ? (
                            ...
                            <img src={previewIcon} alt="Preview" className="w-12 h-12 object-contain drop-shadow-md"
                    ...
                    <div className="flex gap-2">
                         ...
                         {customIconUrl && (
                             <button type="button" onClick={() => setCustomIconUrl('')}
                    ...
                    <input type="file" ref={fileInputRef} className="hidden" accept="image/png, image/jpeg, image/gif, image/webp" onChange={handleFileUpload} />
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-[var(--theme-color)] transition-all" placeholder="Website Name" />
            </div>
            <div>
                ...
                <div className="relative">
                    ...
                    <input type="text" value={url} onChange={(e) => setUrl(e.target.value)}
                        className="w-full pl-10 pr-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-[var(--theme-color)] transition-all" placeholder="https://example.com" />
                </div>
            </div>
            <div>
                ...
                <div className="relative">
                    ...
                    <input type="text" value={customIconUrl.startsWith('data:') ? '' : customIconUrl} onChange={(e) => setCustomIconUrl(e.target.value)}
                        ...
                        placeholder={customIconUrl.startsWith('data:') ? "Using uploaded image..." : "https://example.com/icon.png"} disabled={customIconUrl.startsWith('data:')} />
                </div>
            </div>
            <div className="pt-4 flex gap-3">
                <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 rounded-xl border border-white/10 text-white/70 hover:bg-white/5 hover:text-white transition-all text-sm font-medium">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-2.5 rounded-xl theme-bg hover:brightness-110 text-white font-medium shadow-lg transition-all text-sm flex items-center justify-center gap-2"><Check size={16} /> Save Changes</button>
            </div>
        </form>
      </motion.div>
    </div>
