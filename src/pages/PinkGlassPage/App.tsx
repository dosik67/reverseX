...
const App: React.FC = () => {
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (activeContainer && overContainer && activeContainer !== overContainer) {
       ...
       if (activeContainer === 'top-bar') {
           ...
           setBookmarks([...bookmarks.slice(0, newIndex), itemToMove, ...bookmarks.slice(newIndex, bookmarks.length)]);
       } else {
           setBookmarks(bookmarks.filter(i => i.id !== active.id));
           setTopLinks([...topLinks.slice(0, newIndex), itemToMove, ...topLinks.slice(newIndex, topLinks.length)]);
       }
    }
  };
  ...
  return (
    ...
    >
                ...
                {user ? (
                    <div className="flex items-center gap-3">
                        <div className="text-white/50" title={isSyncing ? "Syncing..." : "Synced"}>
                            {isSyncing ? <CloudUpload size={18} className="animate-pulse text-[var(--theme-color)]" /> : <Cloud size={18} />}
                        </div>
                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/20 border border-white/5 glass-panel">
                            <div className="w-6 h-6 rounded-full theme-bg flex items-center justify-center text-xs font-bold text-white shadow-lg">{user.email?.charAt(0).toUpperCase()}</div>
                            <button onClick={handleLogout} title="Logout" className="text-white/70 hover:text-white transition-colors"><LogOut size={16} /></button>
                        </div>
                    </div>
                ) : (
                    <button onClick={() => setIsAuthOpen(true)} className="p-3 rounded-full hover:bg-white/10 text-white/90 hover:text-white transition-colors" title="Login"><User size={20} /></button>
                )}
            ...
            <main className="flex-1 flex flex-col items-center justify-center -mt-8 px-4 w-full">
                <div className="mb-8 animate-fade-in-down flex justify-center">
                    <a href="https://reversex.vercel.app/" className="cursor-pointer block group" title="ReverseX">
                        <img src="./logo.png" alt="Logo" className="h-24 md:h-28 w-auto drop-shadow-[0_0_15px_rgba(var(--theme-rgb),0.4)] transition-transform duration-500 group-hover:scale-105" />
                    </a>
                </div>
        ...
        <button onClick={() => setIsSettingsOpen(true)} className="fixed bottom-6 right-6 p-3 rounded-full bg-black/20 hover:bg-[rgba(var(--theme-rgb),0.4)] backdrop-blur-md text-white/70 hover:text-white transition-all duration-300 z-40 border border-white/5 hover:rotate-90" title="Settings">
            <Settings size={20} />
        </button>
        ...
        <DragOverlay dropAnimation={dropAnimation}>
            {activeId && activeItem ? (
                <SortableBookmark bookmark={activeItem} variant={findContainer(activeId) === 'top-bar' ? 'pill' : 'card'} onRemove={() => {}} isOverlay />
            ) : null}
        </DragOverlay>
        </div>
    </DndContext>
